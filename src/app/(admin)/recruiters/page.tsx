...
export default function RecruitersPage() {
    ...
    const recruiters = useMemo(() => users.filter(user => user.role === 'Recruiter'), []);

    const filteredAndSortedRecruiters = useMemo(() => {
        let sortedItems = [...recruiters];
        ...
        return sortedItems;
    }, [searchTerm, sortConfig, filters, recruiters]);

    const totalPages = Math.ceil(filteredAndSortedRecruiters.length / ROWS_PER_PAGE);

    const paginatedRecruiters = useMemo(() => {
        const startIndex = (currentPage - 1) * ROWS_PER_PAGE;
        return filteredAndSortedRecruiters.slice(startIndex, startIndex + ROWS_PER_PAGE);
    }, [currentPage, filteredAndSortedRecruiters]);
    ...
    const clearFilters = () => {
        setSearchTerm('');
        setFilters({ status: 'all', employerId: 'all' });
    ...
    return (
        <div>
            <PageHeader title="Recruiter Management">
                 <Button asChild>
                        ...
                        <PopoverContent className="w-80" align="end">
                            <div className="grid gap-4">
                                <div className="space-y-2">
                                    ...
                                    <p className="text-sm text-muted-foreground">Refine recruiter results.</p>
                                </div>
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                ...
                                                <DropdownMenuItem asChild>
                                                    <Link href={`/recruiters/${user.id}/edit`}>
            ...
            <div className="flex items-center justify-between mt-4">
                {isLoading ? (
                    ...
                    <div className="text-sm text-muted-foreground">
                        Showing {Math.min((currentPage - 1) * ROWS_PER_PAGE + 1, filteredAndSortedRecruiters.length)} to {Math.min(currentPage * ROWS_PER_PAGE, filteredAndSortedRecruiters.length)} of {filteredAndSortedRecruiters.length} recruiters.
                    </div>
                )}
