...
import type { JobCategory, Pagination, GetAllParams } from "@/lib/types";
...
import { getJobCategories, deleteJobCategory } from '@/services/api';
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from '@/components/ui/alert-dialog';
import { Checkbox } from '@/components/ui/checkbox';
import { useSearchParams, useRouter } from 'next/navigation';

type SortConfig = {
    key: keyof JobCategory;
    direction: 'asc' | 'desc';
} | null;

const allFieldsConfig = [
    { key: 'name' as const, label: 'Name', sortable: true, sortKey: 'name' as keyof JobCategory },
    { key: 'description' as const, label: 'Description', sortable: true, sortKey: 'description' as keyof JobCategory },
    { key: 'status' as const, label: 'Status', sortable: true, sortKey: 'isActive' as keyof JobCategory },
    { key: 'createdAt' as const, label: 'Created At', sortable: true, sortKey: 'createdAt' as keyof JobCategory },
    { key: 'lastUpdated' as const, label: 'Last Updated', sortable: true, sortKey: 'updatedAt' as keyof JobCategory },
    { key: 'actions' as const, label: 'Actions', sortable: false },
];

type ColumnKeys = typeof allFieldsConfig[number]['key'];
const defaultDisplayFields: ColumnKeys[] = ['name', 'description', 'status', 'lastUpdated'];
...
export default function JobCategoriesPage() {
    ...
    const [rowsPerPageInput, setRowsPerPageInput] = useState<number | string>(10);
     ...
     const fetchCategories = useCallback(() => {
        ...
        const essentialFields = ['id', sortConfig?.key].filter(Boolean) as (keyof JobCategory)[];
    ...
    useEffect(() => {
        ...
        fetchCategories();
    }, [fetchCategories, currentPage, router]);
    ...
    const requestSort = (key: keyof JobCategory) => {
    ...
    const getSortIcon = (key: keyof JobCategory) => {
    ...
    const getEditHref = (categoryId: string) => {
        const params = new URLSearchParams();
        params.set('page', currentPage.toString());
        return `/job-categories/${categoryId}/edit?${params.toString()}`;
    }

    return (
        <div>
            <PageHeader title="Job Categories">
                <Button asChild>
                    <Link href="/job-categories/new">
                            ...
                            {allFieldsConfig.map(col => (
                                columnVisibility[col.key] && (
                                    <TableHead key={col.key} className={cn({
                                        ...
                                        {col.sortable ? (
                                            <Button variant="ghost" onClick={() => requestSort(col.sortKey as keyof JobCategory)} className="px-0 h-auto hover:bg-transparent capitalize">
                                                {col.label} {getSortIcon(col.sortKey as keyof JobCategory)}
                                            </Button>
                                        ) : (
                        ...
                        {isLoading ? (
                            ...
                            categories.map(category => (
                                <TableRow key={category.id}>
                                    ...
                                    {columnVisibility.createdAt && <TableCell className="hidden lg:table-cell">{isValid(category.createdAt) ? format(category.createdAt, 'MMM d, yyyy') : 'N/A'}</TableCell>}
                                    {columnVisibility.lastUpdated && <TableCell>{isValid(category.updatedAt) ? format(category.updatedAt, 'MMM d, yyyy') : 'N/A'}</TableCell>}
                                            ...
                                            <AlertDialog>
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        ...
                                                        <DropdownMenuItem asChild>
                                                            <Link href={`/job-categories/${category.id}`}>
                                                                <Eye className="mr-1 h-4 w-4"/>
                                                                View Details
                                                            </Link>
                                                        </DropdownMenuItem>
                                                        <DropdownMenuItem asChild>
                                                            <Link href={getEditHref(category.id)}>
                                                ...
                                                <AlertDialogContent>
                                                    ...
                                                    <AlertDialogDescription>
                                                        ...
                                                        and may affect job postings associated with it.
                                                    </AlertDialogDescription>
                                                    </AlertDialogHeader>
                                                    <AlertDialogFooter>
