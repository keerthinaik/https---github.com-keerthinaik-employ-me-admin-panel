...
const columnsConfig = [
    ...
    { key: 'companyName' as const, label: 'Company', sortable: true, sortKey: 'companyName' as keyof Job },
    { key: 'status' as const, label: 'Status', sortable: true, sortKey: 'status' as keyof Job },
    { key: 'applications' as const, label: 'Applications', sortable: true, sortKey: 'applications' as keyof Job },
    { key: 'postingDate' as const, label: 'Posted On', sortable: true, sortKey: 'postingDate' as keyof Job },
...
export default function JobsPage() {
    ...
    const SkeletonRow = () => (
        <TableRow>
            ...
            {columnVisibility.companyName && <TableCell className="hidden md:table-cell"><Skeleton className="h-5 w-32" /></TableCell>}
            {columnVisibility.status && <TableCell><Skeleton className="h-6 w-20 rounded-full" /></TableCell>}
            {columnVisibility.applications && <TableCell className="hidden lg:table-cell"><Skeleton className="h-5 w-10" /></TableCell>}
            {columnVisibility.postingDate && <TableCell className="hidden lg:table-cell"><Skeleton className="h-5 w-24" /></TableCell>}
                            ...
                            {columnsConfig.map(col => (
                                columnVisibility[col.key] && (
                                    <TableHead key={col.key} className={cn({
                                        'hidden md:table-cell': col.key === 'companyName',
                                        'hidden lg:table-cell': ['applications', 'postingDate'].includes(col.key),
                                    })}>
                                        {col.sortable ? (
                        ...
                        {isLoading ? (
                            ...
                            paginatedJobs.map(job => (
                                <TableRow key={job.id}>
                                    ...
                                    {columnVisibility.companyName && <TableCell className="text-muted-foreground hidden md:table-cell">{job.companyName}</TableCell>}
                                    {columnVisibility.status && <TableCell>{getStatusBadge(job.status)}</TableCell>}
                                    {columnVisibility.applications && <TableCell className="hidden lg:table-cell">{job.applications}</TableCell>}
                                    {columnVisibility.postingDate && <TableCell className="hidden lg:table-cell">{format(job.postingDate, 'MMM d, yyyy')}</TableCell>}
                                        ...
                                        <TableCell>
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end">
                                                    ...
                                                    <DropdownMenuItem asChild><Link href={`/jobs/${job.id}`}><Eye className="mr-1 h-4 w-4"/> View Details</Link></DropdownMenuItem>
                                                    <DropdownMenuItem asChild><Link href={`/jobs/${job.id}/edit`}><Edit className="mr-1 h-4 w-4"/> Edit</Link></DropdownMenuItem>
                                                    <DropdownMenuItem className="text-destructive focus:text-destructive focus:bg-destructive/10"><Trash2 className="mr-1 h-4 w-4"/> Delete</DropdownMenuItem>
                                                </DropdownMenuContent>
                                            </DropdownMenu>
                                        </TableCell>
                                    )}
