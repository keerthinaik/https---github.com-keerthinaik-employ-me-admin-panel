...
import { createSubAdminUser, updateSubAdminUser, getCountries, getStates, getCities } from '@/services/api';
...
const subAdminUserSchema = z.object({
...
type SubAdminUserFormValues = z.infer<typeof subAdminUserSchema>;
...
export function SubAdminForm({ user }: SubAdminUserFormProps) {
  ...
  const form = useForm<SubAdminUserFormValues>({
    resolver: zodResolver(subAdminUserSchema),
    defaultValues: {
      name: '',
      email: '',
      password: '',
  ...
  const onError = (errors: any) => {
    const firstErrorField = Object.keys(errors)[0] as keyof SubAdminUserFormValues;
    if (firstErrorField) {
  ...
  const onSubmit = async (data: SubAdminUserFormValues) => {
    ...
    try {
      const response = user ? await updateSubAdminUser(user.id, formData) : await createSubAdminUser(formData);
      toast({ title: user ? 'Sub Admin Updated' : 'Sub Admin Created', description: `${response.name} has been successfully saved.` });
        ...
        <form onSubmit={handleSubmit(onSubmit, onError)}>
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="info">
              <Card>
                <CardHeader><CardTitle>Sub Admin Information</CardTitle></CardHeader>
          ...
          <CardFooter className="flex justify-end gap-2 mt-6 px-0">
            ...
            <Button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Saving...' : 'Save Sub Admin'}</Button>
          </CardFooter>
        </form>
      </Form>
